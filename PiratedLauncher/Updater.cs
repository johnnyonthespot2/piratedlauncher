...
namespace PiratedLauncher
{
    public partial class Updater : Form
    {
        ...
        private async Task<string> FetchApiDataAsync(string httpsUrl)
        {
            ...
            {
                client.DefaultRequestHeaders.Add("User-Agent", "PiratedLauncher");

                try
                {
                    // Try HTTPS first
                    return await client.GetStringAsync(httpsUrl);
                }
        ...
        private async void Updater_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                apiVersion = await FetchApiDataAsync("launcher ver here");
                jsonResponse = await FetchApiDataAsync("json link here");
            }
            catch (Exception ex)
            {
